...
OUTPUT_JSON = "/Users/ethan.jiang/Documents/Capstone/mocap_mediapipe/pipeline_test/triangulation_master_slave2_check.json"
...
def extract_projection_matrices(extrinsics_data):
    ...
    P_slave2 = np.array(extrinsics_data["P_slave2"], dtype=np.float64)
    return {
        "master": P_master,
        "slave2": P_slave2,
    }
...
def main():
    ...
    slave2 = load_json(pose_json_paths["slave2"])

    frames1 = master["frames"]
    frames2 = slave2["frames"]
    ...
    for i in range(n):
        ...
        for kpt_idx in KEYPOINTS_TO_USE:
            ...
            if lm1["visibility"] < MIN_VISIBILITY or lm2["visibility"] < MIN_VISIBILITY:
                ...
                kpt_record["slave2_visibility"] = lm2["visibility"]
                frame_record["keypoints"][str(kpt_idx)] = kpt_record
                continue
            ...
            X = triangulate_two_view(P["master"], P["slave2"], pt1, pt2)

            proj1 = reproject_point(P["master"], X)
            proj2 = reproject_point(P["slave2"], X)
